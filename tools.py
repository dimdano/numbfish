...
# Nothing from here is imported into numbfish.py which is entirely self-sufficient
################################################################################

# Numbfish doesn't have to know about colors, but for more advanced things, such
...
def can_kill_king(pos):
    ...
    return any(pos.value(m) >= numbfish.MATE_LOWER for m in pos.gen_moves())

def mrender(pos, m):
    # Numbfish always assumes promotion to queen
    p = 'q' if numbfish.A8 <= m[1] <= numbfish.H8 and pos.board[m[0]] == 'P' else ''
    m = m if get_color(pos) == WHITE else (119-m[0], 119-m[1])
    return numbfish.render(m[0]) + numbfish.render(m[1]) + p

def mparse(color, move):
    m = (numbfish.parse(move[0:2]), numbfish.parse(move[2:4]))
    return m if color == WHITE else (119-m[0], 119-m[1])

def renderSAN(pos, move):
    ''' Assumes board is rotated to position of current player '''
    i, j = move
    csrc, cdst = numbfish.render(i), numbfish.render(j)
    # Rotate flor black
    if get_color(pos) == BLACK:
        csrc, cdst = numbfish.render(119-i), numbfish.render(119-j)
    # Check
    pos1 = pos.move(move)
    ...
    # Pawn moves
    if pos.board[i] == 'P':
        pro = '=Q' if numbfish.A8 <= j <= numbfish.H8 else ''
    ...
    srcs_file = [a for a in srcs if (a - numbfish.A1) % 10 == (i - numbfish.A1) % 10]
    srcs_rank = [a for a in srcs if (a - numbfish.A1) // 10 == (i - numbfish.A1) // 10]
...
def parseSAN(pos, msan):
    ...
    if pawn:
        assert not re.search('[RBN]$', msan), 'Numbfish only supports queen promotion in {}'.format(msan)
    ...
    for (i, j), _ in gen_legal_moves(pos):
        if get_color(pos) == WHITE:
            csrc, cdst = numbfish.render(i), numbfish.render(j)
        else: csrc, cdst = numbfish.render(119-i), numbfish.render(119-j)
...
# returns (from_sq, from_pc, to_sq, to_pc) in chess format
def chess_move_from_to (pos, m):
    ...
    #numbfish assumes this side's pieces are always in capital
...
def get_color(pos):
    ''' A slightly hacky way to to get the color from a numbfish position '''
...
def parseFEN(fen):
    ...
    ep = numbfish.parse(enpas) if enpas != '-' else 0
    score = sum(numbfish.pst[p][i] for i,p in enumerate(board) if p.isupper())
    score -= sum(numbfish.pst[p.upper()][119-i] for i,p in enumerate(board) if p.islower())
    pos = numbfish.Position(board, score, wc, bc, ep, 0)
...
def renderFEN(pos, half_move_clock=0, full_move_clock=1):
    ...
    ep = numbfish.render(pos.ep) if not pos.board[pos.ep].isspace() else '-'
