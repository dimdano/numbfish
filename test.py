...
class Tests(unittest.TestCase):
    ...
    def test_fen(self):
        ...
        for fen in open(fen_file):
            ...
            self.assertEqual(fen, fen1, "Numbfish didn't correctly reproduce the FEN."
                    + repr(pos))

    def test_fen2(self):
        initial = numbfish.Position(numbfish.initial, 0, (True,True), (True,True), 0, 0)
        for pos in tools.flatten_tree(tools.expand_position(initial),3):
            ...
            self.assertEqual(pos.board, pos1.board, "Numbfish didn't correctly reproduce the board")
        ...
        for line in open(pgn_file):
            ...
            for i, msan in enumerate(msans):
                if re.search('=[BNR]', msan):
                    # Numbfish doesn't support underpromotion
                    break
                move = tools.parseSAN(pos, msan)
                msan_back = tools.renderSAN(pos, move)
                self.assertEqual(msan_back, msan,
                                 "Numbfish didn't correctly reproduce the SAN move")
                pos = pos.move(move)

    def test_selfplay(self):
        pos = tools.parseFEN(tools.FEN_INITIAL)
        for d in range(200):
            m, score, _ = tools.search(numbfish.Searcher(), pos, .1)
            if m is None:
                self.assertTrue(score == 0 or abs(score) >= numbfish.MATE_LOWER)
                break
            pos = pos.move(m)
    ...
    def test_value(self):
        for pos in self.positions:
            ...
            for i,p in enumerate(pos.board):
                if p.isupper(): score += numbfish.pst[p][i]
                if p.islower(): score -= numbfish.pst[p.upper()][119-i]
    ...
    def test_3fold(self):
        ...
        with open(path_dont) as file:
            for i, (_pgn, pos_moves) in enumerate(tools.readPGN(file)):
                ...
                if last_pos.score < 0:
                    continue
                move, score, _ = tools.search(numbfish.Searcher(), pos, secs=.1, history=history[:-1])
                if move == last_move:
...
def benchmark(cnt=20, depth=3):
    ...
    for i, line in enumerate(random.sample(list(open(path)), cnt)):
        ...
        searcher = numbfish.Searcher()
        start1 = time.time()
        for search_depth, _, _, _ in searcher.search(pos, 0):
...
def play(version1_version2_secs_plus_fen):
    ...
    for d in range(200):
        ...
        # Test move
        is_dead = lambda pos: any(pos.value(m) >= numbfish.MATE_LOWER for m in pos.gen_moves())
...
def test_xboard(python='python3', verbose=True):
    ...
    try:
        ...
        wait_for('move ')
        write('quit')
        with timeout(5, 'quit did not terminate numbfish'):
            fish.wait()
    finally:
...
def allperft(f, depth=3, verbose=True):
    import gc
    lines = f.readlines()
...
def allmate(path):
    with open(path) as f:
        for line in f:
            ...
            pos = tools.parseFEN(line)
            _, score, _ = tools.search(numbfish.Searcher(), pos, secs=3600)
            if score < numbfish.MATE_LOWER:
...
def quickmate(f, min_depth=1):
    ...
    for line in f:
        ...
        for d in range(min_depth, 99):
            score = searcher.bound(pos, numbfish.MATE_LOWER, d, root=True)
            if score >= numbfish.MATE_LOWER:
...
def main():
    parser = argparse.ArgumentParser(
        description='Run various tests for speed and correctness of numbfish.')
    ...
    p = subparsers.add_parser('selfplay',
        help='run a simple visual numbfish vs numbfish game.')
    ...
    p = subparsers.add_parser('arena',
        help='run a number of games between two numbfish versions.')
    p.add_argument('fish1', type=str, help='numbfish')
    p.add_argument('fish2', type=str, help='numbfish2')
